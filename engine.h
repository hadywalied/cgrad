...
public:
    ...
    Value(T data, std::string label = "",
          std::string operation = "",
          std::vector<std::shared_ptr<Value<T> > > children_vector = {}) : data(data),
                                                                           grad(0.0),
    ...
    friend std::shared_ptr<Value<T> > operator+(const std::shared_ptr<Value<T> > &lhs,
    ...
    friend std::shared_ptr<Value<T> > operator+(const std::shared_ptr<Value<T> > &lhs, const T &rhs) {
        return lhs + std::make_shared<Value<T> >(rhs);
    }

    friend std::shared_ptr<Value<T> > operator+(const T &lhs, const std::shared_ptr<Value<T> > &rhs) {
    ...
    friend std::shared_ptr<Value<T> > tanh(const std::shared_ptr<Value<T> > &val) {
...
template<typename T>
std::ostream &operator<<(std::ostream &os, const std::shared_ptr<Value<T> > &v) {
    auto value = *v;
    os << "Value(label= " << value.label << ", data=" << value.data << ", grad=" << value.grad << ", op= " << value.
            operation << ")";
    return os;
